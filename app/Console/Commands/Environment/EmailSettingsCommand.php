...
class EmailSettingsCommand extends Command
{
    ...
    protected $signature = 'p:environment:mail
                            ...
                            {--email= : Endereço de e-mail de onde as mensagens do Painel serão originadas.}
                            {--from= : Os e-mails de nome do painel parecerão ser de.}
                            {--encryption=}
                            {--host=}
                            {--port=}
                            {--endpoint=}
    ...
    public function handle()
    {
        ...
        $this->line('Atualizando o arquivo de configuração do environment(ENV=ambiente) armazenado.');
        $this->line('');
    }
